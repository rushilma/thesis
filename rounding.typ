...
In this case, let $hat(alg)(g) = hat(x) in S(E;g)$ be the (unique) nearby good solution. This solution is unique as our process for choosing $hat(x)$ implies taking the one which maximizes energy, and two solutions have the same energy with low probability.
