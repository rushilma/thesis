...
First, define $p_"solve"$ as the probability that the algorithm solves a single random instance:
$ p_"solve" = PP(alg(g) in Soln(g)). $ <eq_def_psolve>
...
#proof[
  ...
  $
    p_"cond" (x) <= exp_2 (-E + -1/2 log_2 (epsilon) + 2 eta log_2 (1 / eta) N + O(log_2 N)). #qedhere
  $
]
...
#proof[
  ...
  $
    p_"cond" (x) <= exp_2 (-E + 2 eta log_2 (1 / eta) N + O( 1 )). #qedhere
  $
]
